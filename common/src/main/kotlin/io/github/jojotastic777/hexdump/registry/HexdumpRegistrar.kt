package io.github.jojotastic777.hexdump.registry
...
typealias RegistrarEntry<T> = HexdumpRegistrar<T>.Entry<out T>

abstract class HexdumpRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Hexdump.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexdumpRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
