...
public class ForgeMixinConfigPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (mixinClassName.equals("io.github.jojotastic777.hexdump.forge.mixin.MixinDatagenMain")) {
            var shouldApply = System.getProperty("hexdump.apply-datagen-mixin", "false").equals("true");
            if (shouldApply) {
                Hexdump.LOGGER.warn("Applying scuffed datagen mixin. This should not happen if not running datagen!");
            }
            return shouldApply;
        }
        return true;
